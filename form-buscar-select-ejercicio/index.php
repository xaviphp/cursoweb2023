<?php 
	...
	if ($enviado){
		try {
			...
			$statement = $conexion->prepare('SELECT * FROM clientes WHERE CÓDIGOCLIENTE = :id');
			$statement->execute(
				array(':id'=> $id)
			);
			...
			if($resultados){
				...
				echo "CÓDIGOCLIENTE:" . $resultados['CÓDIGOCLIENTE'] . ' - ' . $resultados['EMPRESA']. ' - ' . $resultados['DIRECCIÓN']. ' - ' . $resultados['POBLACIÓN'] . ' - ' . $resultados['TELÉFONO'] . ' - ' . $resultados['RESPONSABLE'];
				echo "</p>";
